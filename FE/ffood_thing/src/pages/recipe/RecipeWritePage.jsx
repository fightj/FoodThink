...
function RecipeWritePage() {
  ...
  const removeStep = (index) => {
    setSteps(prevSteps =>
      prevSteps.filter((_, i) => i !== index)
        .map((step, i) => ({ ...step, processOrder: i + 1 })) // 순서 재정렬
    );
  };
  ...
  const handleStepImageUpload = (event, index) => {
    ...
    setSteps((prevSteps) =>
      prevSteps.map((step, i) =>
        i === index ? { ...step, imageFile: file } : step
      )
    );
  };
  ...
  const removeStepImage = (index) => {
    setSteps((prevSteps) =>
      prevSteps.map((step, i) =>
        i === index ? { ...step, imageFile: null } : step
      )
    );
  };
                    ...
                    {step.imageFile ? (
                      <>
                        <img
                          ...
                          alt={`Step ${index + 1}`}
                          className="step-uploaded-image"
                        />
                        <button
                          type="button"
                          className="remove-step-image-btn"
